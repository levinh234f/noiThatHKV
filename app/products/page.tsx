    ...
    <main className="bg-[#f6f6f6] text-[#171717]">
      ...
      <section id="danh-sach" className="mx-auto grid max-w-[1280px] gap-7 px-4 py-14 sm:px-6 lg:grid-cols-[220px_1fr] xl:px-0">
        ...
        <div className="min-w-0">
          ...
          {loading ? <div className="grid grid-cols-2 gap-3 pt-7 sm:gap-4 md:grid-cols-3 xl:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="aspect-[3/4] animate-pulse rounded-[15px] bg-[#e5e5e2]" />)}</div> : visible.length ? <div className="grid grid-cols-2 gap-3 pt-7 sm:gap-4 md:grid-cols-3 xl:grid-cols-4">{visible.map((product) => <ProductCard key={product.id} product={toCard(product)} mobileFit />)}</div> : <div className="my-16 rounded-[15px] bg-white p-12 text-center"><h2 className="text-xl">Không tìm thấy sản phẩm</h2><button type="button" onClick={() => setSelectedType("Tất cả")} className="mt-5 rounded-full bg-[#6b7d65] px-6 py-3 text-sm text-white">Xóa bộ lọc</button></div>}
